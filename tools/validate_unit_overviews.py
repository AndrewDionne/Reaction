...
def main() -> int:
    ...
    for ov in notes.get('unitOverviews', []):
        ...
        for sec_idx, vocab_section in enumerate(target.get('vocabulary', []), start=1):
            if not vocab_section.get('title'):
                errors.append(f'{unit}.vocabulary[{sec_idx}]: missing title')
            for term_idx, vocab_term in enumerate(vocab_section.get('terms', []), start=1):
                if isinstance(vocab_term, dict):
                    term = str(vocab_term.get('term', '')).strip()
                    definition = str(vocab_term.get('definition', '')).strip()
                else:
                    term = str(vocab_term).strip()
                    definition = ''
                if not term:
                    errors.append(f'{unit}.vocabulary[{sec_idx}].terms[{term_idx}]: missing term')
                if not definition:
                    errors.append(f'{unit}.vocabulary[{sec_idx}].terms[{term_idx}]: missing definition for {term or "term"}')
